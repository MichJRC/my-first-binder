...
def load_tiff_overlay(tiff_path):
    ...
    try:
        with rasterio.open(tiff_path) as src:
            ...
            # Process data for 1-100 range (0 is no-data)
            display_data = final_data.copy()
            
            # Exclude 0 values (0 represents no-data)
            valid_mask = (display_data >= 1) & (display_data <= 100)
            
            # Choose colormap based on your data type
            from matplotlib.colors import Normalize
            import matplotlib.cm as cm
            
            # Select appropriate colormap and invert it (1=brightest, 100=darkest):
            cmap = cm.get_cmap('viridis_r')        # Good for general continuous data (inverted)
            # cmap = cm.get_cmap('RdYlGn_r')       # Good for vegetation indices (inverted)
            # cmap = cm.get_cmap('terrain_r')      # Good for elevation (inverted)
            # cmap = cm.get_cmap('Blues_r')        # Good for water-related data (inverted)
            # cmap = cm.get_cmap('coolwarm_r')     # Good for temperature data (inverted)
            
            # Set normalization range (1-100 since 0 is no-data)
            norm = Normalize(vmin=1, vmax=100)
            
            # Create masked array to handle invalid values
            masked_data = np.ma.masked_where(~valid_mask, display_data)
            
            # Display the data with colormap
            im = ax.imshow(masked_data, extent=extent, cmap=cmap, norm=norm, alpha=0.7)
...
if __name__ == '__main__':
    ...
    TIFF_FILE = "data/MGRS-32TPQ_201507-202507_water_occurrence_map_new.tif"  # 👈 TIFF FILE PATH (0=no-data, 1-100=data)
